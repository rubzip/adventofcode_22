def create_value(val, divisors):
    result = dict()
    for divisor in divisors:
        result[divisor] = val%divisor
    return result
...
divisors = list(map(lambda x: x['test'], monkeys))
for i in range(num_of_monkeys):
    new_items = list()
    for item in monkeys[i]['starting_items']:
        new_items.append(create_value(item, divisors))
    
    monkeys[i]['starting_items'] = new_items
